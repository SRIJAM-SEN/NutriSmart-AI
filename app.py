...
def generate_day_meal(food_df, max_calories, scaler, use_ml=True, tolerance=0.15):
    ...
    while (total_calories < lower or total_calories > upper) and attempts < 80:
        ...
        opts = food_df[
            food_df["Meal_Type"].str.lower() == mt.lower()
        ]
...
def meal_plan():
    ...
    if request.method == "POST":
        ...
        dislikes_list = [
            d.strip() for d in dislikes_text.split(",") if d.strip()
        ]
        ...
        if filtered.empty:
            ...
            message = "No items left after applying filters/dislikes."

        else:

            week_plan = generate_meal_plan_week(
                filtered,
                max_calories,
                use_ml=use_ml
            )
